 ...
 * For those who want to hold a USD-pegged stablecoin, for whatever reason, it will be better to diversify your risk across a several different stablecoins in one basket, represented by one coin, one which can always be reedemed for the current market value in proportion to the collateral you deposited, especially one that promises:
 * <b>WE WILL BAIL YOU OUT if one of your collateralized stablecoins breaks its peg for good and fails. As we grow, we intend to build a bailout fund with a USD value greater than the total USD value of any single coin held in our collateral. More details about this below.</b>
